...
export default function EditCoursePage({
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const course: FeaturedCourse = data.course
          setName(course.name)
          setSlug(course.slug)
          setFullName(course.fullName)
          setApiCourseName(course.apiCourseName)
          setType(course.type)
          setNivel(course.nivel)
          setDescription(course.description)
          setLongDescription(course.longDescription)
          setDuration(course.duration)
          setAverageSalary(course.averageSalary)
          setMarketDemand(course.marketDemand)
          setImageUrl(course.imageUrl)
          setAreas(course.areas || [])
          setSkills(course.skills || [])
          setCareerPaths(course.careerPaths || [])
    ...
    fetchCourse()
  }, [firebaseUser, hasPermission, courseId])
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500">Atualize as informações do curso</p>
        </div>
      </div>
