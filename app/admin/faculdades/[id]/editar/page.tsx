...
export default function EditarFaculdadePage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setForm({
            ...
            longDescription: inst.longDescription || '',
            founded: inst.founded ? String(inst.founded) : '',
            type: inst.type || 'PRIVADA',
            campusCount: inst.campusCount ? String(inst.campusCount) : '',
            studentCount: inst.studentCount || '',
            coursesOffered: inst.coursesOffered ? String(inst.coursesOffered) : '',
            headquartersCity: inst.headquartersCity || '',
            headquartersState: inst.headquartersState || '',
            mecRating: inst.mecRating ? String(inst.mecRating) : '',
            emecLink: inst.emecLink || '',
            modalities: inst.modalities || [],
            academicLevels: inst.academicLevels || [],
            highlights: inst.highlights || [],
            logoUrl: inst.logoUrl || '',
            imageUrl: inst.imageUrl || '',
            imageAlt: inst.imageAlt || '',
            keywords: inst.keywords || [],
            metaTitle: inst.metaTitle || '',
            metaDescription: inst.metaDescription || '',
            isActive: inst.isActive ?? true,
          })
        } else {
          setError('Erro ao carregar faculdade')
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setError(data.error || 'Erro ao atualizar faculdade')
      }
    } catch (err) {
      console.error('Error updating institution:', err)
      setError('Erro ao atualizar faculdade')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Editar {form.name || 'Faculdade'}
          </h1>
          <p className="text-gray-500">Atualizar dados da faculdade parceira</p>
        </div>
      </div>
