...
const HeaderNew: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[1000] transition-all duration-500 ${
      scrolled ? '' : 'bg-gradient-to-r from-bolsa-primary to-blue-700'
    }`}>
      <div
      ...
      >
        <div
          className={`flex items-center justify-between w-full mx-auto transition-all duration-500 ${
            scrolled ? 'bg-white px-10 py-4 rounded-full' : 'px-4'
          }`}
        >
          ...
          <nav className="hidden lg:flex gap-10 items-center">
            ...
            {/* User Menu - Show login button by default, user menu when logged in */}
            {user ? (
                ...
                >
                  {user.avatar ? (
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      scrolled ? 'bg-bolsa-primary text-white' : 'bg-white/20'
                    }`}>
                      <User size={18} />
                    </div>
                  )}
