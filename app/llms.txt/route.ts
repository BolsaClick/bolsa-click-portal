...
export async function GET() {
  ...
  lines.push('')
  lines.push('## Imagens de compartilhamento')
  lines.push('')
  lines.push(`Toda página de curso, curso por cidade, categoria (graduação/pós-graduação/profissionalizantes), a home e cada post do blog têm uma imagem de compartilhamento própria (1200x630), gerada a partir do mesmo dado real exibido na página — nome do curso, cidade, faixa de preço e o teto de desconto verificado de ${DISCOUNT_CEILING_PCT}%. Nunca uma foto de banco de imagens ou número decorativo. Cada página referencia a sua no meta tag og:image e no campo \`image\` (ImageObject) do JSON-LD; o texto renderizado na imagem é legível e corresponde ao \`caption\` do schema. Exemplos: https://www.bolsaclick.com.br/opengraph-image (home), https://www.bolsaclick.com.br/bolsas-de-estudo/opengraph-image, https://www.bolsaclick.com.br/graduacao/opengraph-image.`)
  lines.push('')
  lines.push('## Política de uso')
  lines.push('')
  lines.push('Crawlers de IA (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, OAI-SearchBot, Applebot-Extended, etc) têm acesso autorizado a todo conteúdo público, incluindo as imagens de compartilhamento acima (`/api/og/` é liberado mesmo dentro do `/api/`, que é privado no restante). Páginas privadas (/admin, /checkout, /api, /minha-conta, etc) são bloqueadas via robots.txt.')
  lines.push('')
  lines.push('## Contato')
  lines.push('')
